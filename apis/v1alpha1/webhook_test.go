...
func TestCreateCrdbCluster(t *testing.T) {
	...
	for _, testcase := range testcases {
		_, err := testcase.Cluster.ValidateCreate(ctx, nil)
		if testcase.ErrMsg == "" {
...
func TestUpdateCrdbCluster(t *testing.T) {
	...
	ctx := context.Background()
	for _, testcase := range testcases {
		_, err := testcase.Cluster.ValidateUpdate(ctx, &oldCluster, nil)
		require.Error(t, err)
		require.Equal(t, err.Error(), testcase.ErrMsg)
	}
}
...
func TestUpdateCrdbClusterLabels(t *testing.T) {
	...
	for _, tc := range testcases {
		_, err := tc.Cluster.ValidateUpdate(ctx, &oldCluster, nil)
		if tc.ShouldError {
			require.Error(t, err)
