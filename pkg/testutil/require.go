...
func RequireAtMostOneVersionCheckerJob(t *testing.T, sb testenv.DiffingSandbox, timeout time.Duration) {
	...
	ctxWithTimeout, cancel := context.WithTimeout(context.Background(), timeout)
	defer cancel()
	err := wait.PollUntilContextCancel(ctxWithTimeout, 10*time.Second, true, func(ctxWithTimeout context.Context) (bool, error) {
...
func RequireDbContainersToUseImage(t *testing.T, sb testenv.DiffingSandbox, cr *api.CrdbCluster) {
	ctxWithTimeout, cancel := context.WithTimeout(context.Background(), 600*time.Second)
	defer cancel()
	err := wait.PollUntilContextCancel(ctxWithTimeout, 10*time.Second, true, func(ctxWithTimeout context.Context) (bool, error) {
...
func RequireDecommissionNode(t *testing.T, sb testenv.DiffingSandbox, b ClusterBuilder, numNodes int32) {
	ctxWithTimeout, cancel := context.WithTimeout(context.Background(), 700*time.Second)
	defer cancel()
	cluster := b.Cluster()
...
func RequireClusterInImagePullBackoff(t *testing.T, sb testenv.DiffingSandbox, b ClusterBuilder) {
	...
	defer cancel()
	wErr := wait.PollUntilContextCancel(ctxWithTimeout, 10*time.Second, true, func(ctxWithTimeout context.Context) (bool, error) {
