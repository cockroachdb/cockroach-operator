...
func TestSQLIngressBuilder(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "builds v1beta1 sql ingress",
			cluster: cluster.WithIngress(&v1alpha1.IngressConfig{
				SQL: &v1alpha1.Ingress{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := resource.SQLIngressBuilder{
				Cluster:   tt.cluster,
				Labels:    selector,
